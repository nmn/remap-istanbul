...
define([
	...
	return function remap(coverage, options) {
		...
		forEach(coverage, function (item) {
			forEach(Object.keys(item), function (filePath) {
				...
				if (sourceMap.sourcesContent && sourceStore) {
					forEach(sourceMap.sourcesContent, function (source, idx) {
						sourceStore.set(path.join(sourceMapDir, sourceMap.sources[idx]), source);
					});
				}

				forEach(Object.keys(fileCoverage.fnMap), function (index) {
				...
				forEach(Object.keys(fileCoverage.statementMap), function (index) {
				...
				forEach(Object.keys(fileCoverage.branchMap), function (index) {
